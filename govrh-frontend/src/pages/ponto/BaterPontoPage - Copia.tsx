...
function useServerStatus(intervalMs = 15000) {
  ...
  const check = useCallback(async () => {
    const t0 = Date.now()
    try {
      await fetch('/api/v1/health', { method: 'GET', cache: 'no-store', signal: AbortSignal.timeout(5000) })
...
export default function BaterPontoPage() {
  ...
  const handleCapturar = useCallback(async () => {
    ...
    if (serverStatus === 'offline') {
      toast({ variant: 'destructive', title: 'Servidor indisponível' })
      return
    }
          ...
          <div className="p-5 space-y-4">
            ...
            {serverStatus === 'offline' && (
              <div className="bg-red-50 border border-red-200 rounded-xl p-3 flex items-center gap-2">
                ...
                <button onClick={recheck} className="text-xs text-red-500 underline underline-offset-2 shrink-0 touch-manipulation">Tentar</button>
              </div>
            )}
